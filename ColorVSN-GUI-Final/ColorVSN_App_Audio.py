...
def apply_night_vision(frame):
    ...
    processed_frame = brightness_contrast_effect(frame)

    # Apply infrared effect
    if bool(infrared):
        processed_frame = infared_effect(processed_frame)
        # Apply colormap effect
    if colormap is not None:
        processed_frame = apply_colormap(processed_frame)
...
def main():
    ...
    out = cv2.VideoWriter('Future-1-output.mp4', codec, fps, (width, height))
    ...
    processed_clip = VideoFileClip(input_video_path)
    final_clip = processed_clip.set_audio(original_clip.audio)
    final_clip.write_videofile('test-output.mp4', codec='libx264', audio_codec='aac')
